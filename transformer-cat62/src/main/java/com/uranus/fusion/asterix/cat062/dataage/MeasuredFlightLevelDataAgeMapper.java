...
class MeasuredFlightLevelDataAgeMapper {
  ...
  public static DataAge read(List<Byte> uap, DataSpec dataSpec) {
    ...
    DpIndicator dpIndicator =
        dataSpec.getDpIndicator(TrackDataAgeConfig.MEAUSRED_FLIGHT_LEVEL_AGE_DRN);
    if (dpIndicator.getIndication().equals(DpIndicationEnum.PRESENCE)) {
      dpIndicator.setSize(TrackDataAgeConfig.MEAUSRED_FLIGHT_LEVEL_AGE_SIZE);

      int index =
          dataSpec.calculateOctetIndexByDrn(TrackDataAgeConfig.MEAUSRED_FLIGHT_LEVEL_AGE_DRN);
