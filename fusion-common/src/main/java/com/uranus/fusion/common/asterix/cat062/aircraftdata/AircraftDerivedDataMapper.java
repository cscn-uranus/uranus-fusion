...
import com.uranus.fusion.common.asterix.cat062.aircraftdata.sub.*;
...
public class AircraftDerivedDataMapper {
  ...
  public static AircraftDerivedData read(List<Byte> message, FieldSpec fieldSpec) {
    ...
    if (fpIndicator.getIndication().equals(FpIndicationEnum.PRESENCE)) {
      ...
      TrackAngle trackAngle = TrackAngleMapper.readTrackAngle(message, dataSpec);
      ...
      EmitterCategoryEnum emitterCategoryEnum =
          EmitterCategoryMapper.readEmitterCategory(message, dataSpec);
      aircraftDerivedData.setEmitterCategory(emitterCategoryEnum);

      Wgs84Position position = PositionMapper.readPosition(message, dataSpec);
      aircraftDerivedData.setPosition(position);

      GeometricAltitude geometricAltitude =
          GeometricAltitudeMapper.readGeometricAltitude(message, dataSpec);
      ...
      ModeSelMbData modeSelMbData = ModeSelMapper.readModeSMbData(message, dataSpec);
      ...
      BarometricPressureSetting barometricPressureSetting =
          BarometricPressureSettingMapper.readBarometricPressureSetting(message, dataSpec);
