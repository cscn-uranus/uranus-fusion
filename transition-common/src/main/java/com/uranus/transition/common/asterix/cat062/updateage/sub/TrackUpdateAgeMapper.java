...
public class TrackUpdateAgeMapper {
  ...
  public static UpdateAge read(List<Byte> uap, DataSpec dataSpec) {
    ...
    DpIndicator dpIndicator = dataSpec.getDpIndicator(SystemTrackUpdateAgeConfig.TRACK_AGE_DRN);
    if (dpIndicator.getIndication().equals(DpIndicationEnum.PRESENCE)) {
      int index = dataSpec.calculateOctetIndexByDrn(SystemTrackUpdateAgeConfig.TRACK_AGE_DRN);
      UpdateAge updateAge = new UpdateAge();

      dpIndicator.setSize(SystemTrackUpdateAgeConfig.TRACK_AGE_SIZE);
