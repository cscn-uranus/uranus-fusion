...
import com.uranus.transition.common.fdexm.dto.ssr.BssrDTO;
...
public class FdexmValidator {
  ...
  public static boolean isValidAddsSsrInfoDom(BssrDTO dom) {
    if (null == dom.getTitle()
        || null == dom.getSendingTime()
        || null == dom.getSsrCode()
        || null == dom.getOperation()
        || null == dom.getOperationTime()
        || null == dom.getActualDepartureAerodrome()
        || null == dom.getActualDestinationAerodrome()
        || null == dom.getFlightIdentification()
        || null == dom.getEstimatedOffBlockDate()
        || null == dom.getEstimatedOffBlockTime()) {
      return false;
    }
    boolean value = true;
  ...
  public static boolean isValidAddsMessageDom(FdexmDTO dom) {
    return null != dom.getTitle() && null != dom.getSendingTime();
  }
