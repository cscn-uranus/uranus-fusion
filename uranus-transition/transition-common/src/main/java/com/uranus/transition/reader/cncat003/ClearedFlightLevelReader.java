 ...
 * @date 2019/6/28
 */
public class ClearedFlightLevelReader {

  public static FlightLevel read(List<Byte> message, FieldSpec fieldSpec) {

    FpIndicator fpIndicator = fieldSpec.findFpIndicatorByFrn(CnCat003Config.CLEARED_FLIGHT_LEVEL_FRN);
    if (fpIndicator.getIndication().equals(FpIndicationEnum.PRESENCE)) {
      fpIndicator.setSize(CnCat003Config.CLEARED_FLIGHT_LEVEL_SIZE);

      int index = fieldSpec.calculateIndexByFrn(CnCat003Config.CLEARED_FLIGHT_LEVEL_FRN);

      FlightLevel clearedFlightLevel = new FlightLevel();
