 ...
 */
class TargetSizeAndOrientationReader {

  static TargetSizeAndOrientation read(List<Byte> uap, FieldSpec fieldSpec) {
    FpIndicator fpIndicator =
        fieldSpec.findFpIndicatorByFrn(EuCat062Config.TARGET_SIZE_AND_ORIENTATION_FRN);
    if (fpIndicator.getIndication().equals(FpIndicationEnum.PRESENCE)) {
      int sizeCount = EuCat062Config.TARGET_SIZE_AND_ORIENTATION_EX_SIZE;
      int index = fieldSpec.calculateIndexByFrn(EuCat062Config.TARGET_SIZE_AND_ORIENTATION_FRN);
