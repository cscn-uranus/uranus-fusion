 ...
 * @date 2019/6/28
 */
public class ReportFlightLevelReader {

  public static FlightLevel read(List<Byte> message, FieldSpec fieldSpec) {

    FpIndicator fpIndicator = fieldSpec.findFpIndicatorByFrn(CnCat003Config.REPORTED_FLIGHT_LEVEL_FRN);
    if (fpIndicator.getIndication().equals(FpIndicationEnum.PRESENCE)) {
      fpIndicator.setSize(CnCat003Config.REPORTED_FLIGHT_LEVEL_SIZE);

      int index = fieldSpec.calculateIndexByFrn(CnCat003Config.REPORTED_FLIGHT_LEVEL_FRN);

      FlightLevel reportFlightLevel = new FlightLevel();
      ...
      reportFlightLevel.setFl(fl);

      return reportFlightLevel;
    }
    return null;
  }
}
