...
import com.uranus.transition.common.asterix.uap.shared.datainfo.TrackTypeEnum;
import com.uranus.transition.common.asterix.uap.shared.emitter.InterrogationStatusEnum;
import com.uranus.transition.common.asterix.uap.shared.emitter.adsb.AdsBroadcastConsistentStatusEnum;
import com.uranus.transition.common.asterix.uap.shared.datainfo.CorrelatedStatusEnum;
import com.uranus.transition.common.asterix.uap.shared.flightplan.FplDataStatusEnum;
import com.uranus.transition.common.asterix.uap.shared.measure.altitude.CalculatedAltitudeSourceEnum;
import com.uranus.transition.common.asterix.uap.shared.measure.altitude.MostReliableAltitudeStatusEnum;
 ...
 */
class TrackStatusReader {

  static TrackStatus readTrackStatus(List<Byte> message, FieldSpec fieldSpec) {

    int sizeCount = EuCat062Config.TRACK_STATUS_EX_SIZE;

    FpIndicator fpIndicator = fieldSpec.findFpIndicatorByFrn(EuCat062Config.TRACK_STATUS_FRN);
    if (fpIndicator.getIndication().equals(FpIndicationEnum.PRESENCE)) {
      int index = fieldSpec.calculateIndexByFrn(EuCat062Config.TRACK_STATUS_FRN);
      ...
      sizeCount+=EuCat062Config.TRACK_STATUS_EX_SIZE;
      ...
      readThirdContent(message.get(index + 3), trackStatus);
      String fx3Bit = ByteUtil.toString(message.get(index + 3)).substring(7, 8);
      ...
      readFourthContent(message.get(index + 4), trackStatus);
      String fx4Bit = ByteUtil.toString(message.get(index + 4)).substring(7, 8);
      ...
      readFifthContent(message.get(index + 5), trackStatus);
      String fx5Bit = ByteUtil.toString(message.get(index + 5)).substring(7, 8);
  ...
  private static void readFirstContent(Byte octet, TrackStatus trackStatus) {
    ...
    trackStatus.setTrackTypeEnum(
        ByteUtil.ZERO_BIT.equals(simBit) ? TrackTypeEnum.ACTUAL : TrackTypeEnum.SIMULATED);
    ...
    trackStatus.setFlightPlanCorrelatedStatusEnum(
        ByteUtil.ZERO_BIT.equals(fpcBit)
            ? CorrelatedStatusEnum.NOT_CORRELATED
            : CorrelatedStatusEnum.CORRELATED);
