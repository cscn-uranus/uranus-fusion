...
import com.uranus.transition.common.asterix.uap.shared.emitter.mode3.CodeChangeStatusEnum;
...
/**
 * TrackMode3CodeReader
 *
 * @author 肖鹏 tellxp@github.com date 2018/10/15
 */
class TrackMode3CodeReader {

  static Mode3Code read(List<Byte> uapOctetList, FieldSpec fieldSpec) {

    // Track Mode 3/A Code: frn=9, size=2
    FpIndicator fpIndicator = fieldSpec.findFpIndicatorByFrn(EuCat062Config.TRACK_MODE_3A_CODE_FRN);

    if (fpIndicator.getIndication().equals(FpIndicationEnum.PRESENCE)) {
      fpIndicator.setSize(EuCat062Config.TRACK_MODE_3A_CODE_SIZE);

      int index = fieldSpec.calculateIndexByFrn(EuCat062Config.TRACK_MODE_3A_CODE_FRN);
