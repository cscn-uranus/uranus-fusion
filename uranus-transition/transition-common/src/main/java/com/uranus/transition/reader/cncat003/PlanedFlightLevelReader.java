 ...
 * @date 2019/6/28
 */
public class PlanedFlightLevelReader {

  public static FlightLevel read(List<Byte> message, FieldSpec fieldSpec) {

    FpIndicator fpIndicator = fieldSpec.findFpIndicatorByFrn(CnCat003Config.PLANED_FLIGHT_LEVEL_FRN);
    if (fpIndicator.getIndication().equals(FpIndicationEnum.PRESENCE)) {
      fpIndicator.setSize(CnCat003Config.PLANED_FLIGHT_LEVEL_SIZE);

      int index = fieldSpec.calculateIndexByFrn(CnCat003Config.PLANED_FLIGHT_LEVEL_FRN);

      FlightLevel planedFlightLevel = new FlightLevel();
      ...
      planedFlightLevel.setFl(fl);

      return planedFlightLevel;
    }
    return null;
  }
}
